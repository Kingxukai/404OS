#ifndef _FAT32_DISK_H__
#define _FAT32_DISK_H__
...
#define LONG_NAME_CHAR_SET(x) (((uint16_t)(x)) & 0x00FF)
...
/*用于将inode号转换为数组下标索引，因为inode号从1开始，而数组下标索引从0开始，所以需要进行转换，如果inode号为0，则直接返回0，因为0对应的是根目录inode，不需要转换。*/
#define FATINUM_TO_SECTOR(ino) (INUM_TRANSFER(ino) / (FCB_PER_BLOCK) + FirstDataSector)
/*将inode号转换为对应的扇区号，以进行读取文件数据。inode的数组下标（FCB号）/一个扇区的FCB数 + 第一个块数据扇区号，即可得到inode号所在的扇区号。*/
#define FATINUM_TO_OFFSET(ino) (INUM_TRANSFER(ino) % (FCB_PER_BLOCK))
/*将inode号转换为偏移的FCB号*/

// the logistic number of cluster for position : off
// start from 0
#define LOGISTIC_C_NUM(off) ((off) / (__BPB_SecPerClus * __BPB_BytsPerSec))//byte_offest /（(sector_num/cluster） * (Byte_size/sector)) = cluster_num
#define LOGISTIC_C_OFFSET(off) ((off) % (__BPB_SecPerClus * __BPB_BytsPerSec))//byte_offest % ((sector_num/cluster）* __BPB_BytsPerSec) = cluster_offest_byte_num->offest
// the logistic number of sector for position : off
// start form 0
#define LOGISTIC_S_NUM(off) (LOGISTIC_C_OFFSET(off) / __BPB_BytsPerSec)//cluster_offest_byte / (Byte_size/sector) = cluster_offest_sector_num->sector_num
#define LOGISTIC_S_OFFSET(off) (LOGISTIC_C_OFFSET(off) % __BPB_BytsPerSec)//cluster_offest_byte % (Byte_size/sector) = sector_offest_byte_num->offest
...
typedef struct FAT32_BootRecord {
    ...
    // 0xEB 0x?? 0x??
    uint8_t OEMName[8]; // OEM Name Identifier.
    ...
    // 1 or 2
    uint8_t RootEntCnt[2]; // for FAT32 , set to 0
    ...
    uint8_t FATSz16[2];    // On FAT32 volumes this field must be 0, and BPB_FATSz32 contains the FAT size count.
    ...
    uint8_t ExtFlags[2]; //  0-3 : Zero-based number of active FAT
    // 4-6 : Reserved
    ...
    uint8_t VolID[4]; // Volume ID 'Serial' number
    uint8_t VolLab[11];
    // Volume label string.
...
typedef struct FAT32_Fsinfo {
    uint8_t LeadSig[4]; // validate that this is in fact an FSInfo sector
    // Value 0x41615252
    uint8_t Reserved1[480]; // is currently reserved for future expansion
    uint8_t StrucSig[4];    // is more localized in the sector to the location of the fields that are used
    // Value 0x61417272
    ...
    uint8_t TrailSig[4]; // validate that this is in fact an FSInfo sector
    // Value 0xAA550000
...
//     uint8_t day : 5;   // 0~31
//     uint8_t month : 4; // Jan ~ Dec
//     uint8_t year : 7;  // form 1980 (1980~2107)
// } __attribute__((packed)) uint16_t;

// // Time
// typedef struct __time_t {
//     uint8_t second_per_2 : 5; // 2-second increments 0~59
//     uint8_t minute : 6;       // number of minutes 0~59
//     uint8_t hour : 5;         // hours 0~23
// } __attribute__((packed)) uint16_t;

// typedef struct __date_t {
//     uint8_t day;   // 0~31
//     uint8_t month; // Jan ~ Dec
//     uint8_t year;  // form 1980 (1980~2107)
// } __attribute__((packed)) uint16_t;

// // Time
// typedef struct __time_t {
//     uint8_t second_per_2; // 2-second increments 0~59
//     uint8_t minute;       // number of minutes 0~59
//     uint8_t hour;         // hours 0~23
// } __attribute__((packed)) uint16_t;

// Directory Structure (short name)
typedef struct Short_Dir_t {
    uint8_t DIR_Name[FAT_SFN_LENGTH]; // directory name
    uint8_t DIR_Attr;                 // directory attribute
    uint8_t DIR_Dev;                  // reserved, but we use it as DEVICE
    // DIR_NTRes
...
typedef struct Long_Dir_t {
    uint8_t LDIR_Ord;       // The order of this entry in the sequence
    uint16_t LDIR_Name1[5]; // characters 1 through 5
    uint8_t LDIR_Attr;      // Attributes
